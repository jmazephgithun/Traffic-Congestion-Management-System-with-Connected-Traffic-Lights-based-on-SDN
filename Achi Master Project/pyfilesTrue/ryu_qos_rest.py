...
class QosRestSwitch(app_manager.RyuApp):
    ...
    def _install_priority_flow(self, dp):
        ...
        # (optional) also prioritize server replies (udp_src=9999)
        match_reply = p.OFPMatch(eth_type=0x0800, ip_proto=17, udp_src=self.CTRL_UDP_PORT)
        self._add_flow(dp, priority=20000, match=match_reply, actions=actions)
        self.logger.info('[Ryu] Installed reverse priority rule on dpid=%s', dp.id)
    ...
    def _remove_priority_flow(self, dp):
        ...
        self._del_flow(dp, match=p.OFPMatch(eth_type=0x0800, ip_proto=17, udp_src=self.CTRL_UDP_PORT))
